...
class TransactionsPercentileTest {
    ...
    @Test
    void getPercentileValueBasicCases() {
        // Sorted ascending by feePerVSize
        List<Transaction> txs = sortedTxs("1", "2", "3", "4", "5");

        // 0th percentile -> index ceil(0*5)-1 => -1 => max(0, -1) => 0 => first
        assertEquals(new BigDecimal("1"), percentile.getPercentileValue(0.0, txs));

        // 25th percentile -> ceil(0.25*5)-1 => ceil(1.25)-1 => 1
        assertEquals(new BigDecimal("2"), percentile.getPercentileValue(0.25, txs));

        // 75th percentile -> ceil(0.75*5)-1 => ceil(3.75)-1 => 3
        assertEquals(new BigDecimal("4"), percentile.getPercentileValue(0.75, txs));

        // 99th percentile -> ceil(0.99*5)-1 => ceil(4.95)-1 => 4 => last
        assertEquals(new BigDecimal("5"), percentile.getPercentileValue(0.99, txs));

        // 100th percentile -> ceil(1.0*5)-1 => 5-1 => 4
        assertEquals(new BigDecimal("5"), percentile.getPercentileValue(1.0, txs));
    }

    @Test
    void getPercentileValueSingletonList() {
        List<Transaction> txs = sortedTxs("42");
        // Any percentile should yield the only element
        assertEquals(new BigDecimal("42"), percentile.getPercentileValue(0.0, txs));
        assertEquals(new BigDecimal("42"), percentile.getPercentileValue(0.5, txs));
        assertEquals(new BigDecimal("42"), percentile.getPercentileValue(1.0, txs));
    }

    @Test
    void getNumOfOutliersComputesIndex() {
        // For size N, returns ceil(p*N)-1
        assertEquals(0, percentile.getNumOfOutliers(0.99));
        assertEquals(1, percentile.getNumOfOutliers(0.99));
        assertEquals(4, percentile.getNumOfOutliers(0.99));
        assertEquals(98, percentile.getNumOfOutliers(0.99));
        assertEquals(24, percentile.getNumOfOutliers(0.25));
        assertEquals(74, percentile.getNumOfOutliers(0.75));
    }
